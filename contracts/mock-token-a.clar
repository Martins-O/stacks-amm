;; Mock Token A - SIP-010 compliant fungible token for testing
...
(define-fungible-token mock-token-a)

(define-data-var token-name (string-ascii 32) "Mock Token A")
(define-data-var token-symbol (string-ascii 10) "MOCK-A")
...
(begin
  (try! (ft-mint? mock-token-a TOTAL_SUPPLY CONTRACT_OWNER))
)
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance mock-token-a who))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply mock-token-a))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (ft-transfer? mock-token-a amount from to)
  )
)
...
(define-public (mint (amount uint) (to principal))
  (begin
    ...
    (ft-mint? mock-token-a amount to)
  )
)
